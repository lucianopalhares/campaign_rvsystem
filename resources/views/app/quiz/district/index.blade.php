...
@section('main')
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-map"></i> {{isset($title)?$title:'Bairros'}} da Campanha {{$quizCampaign->id}} &nbsp;&nbsp;<a href="{{url('app/campanha/'.$quizCampaign->slug.'/bairros/create')}}" class="text-primary" data-toggle="tooltip" data-placement="right" title="Cadastrar Um Bairro na Campanha"><span class="fa fa-plus"></span></a></h1>
          <p>Tabela com {{isset($title)?$title:'todos os Bairros'}} da Campanha {{$quizCampaign->id}}</p>
        </div>
                ...
                <table class="table table-hover table-bordered" id="sampleTable">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>              
                      <a href="{{url('app/campanha/'.$quizCampaign->slug.'/bairros/'.$item->id.'/edit')}}" class="text-primary"><span class="fa fa-edit" data-toggle="tooltip" data-placement="left" title="" data-original-title="Editar"></span></a>
                      @if(!$item->answers()->exists()&&!$item->questions()->exists()&&!$item->options()->exists())
                      <form name="delete" action="{{url('app/campanha/'.$quizCampaign->slug.'/bairros/'.$item->id)}}" method="POST" style="display: none;">
                            @method('DELETE')
                            @csrf
                        </form>                    
