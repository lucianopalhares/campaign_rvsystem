...
@section('main')
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-user-o"></i> Faltas de Alunos</h1>
          <p>{{isset($show)?'Ver':'Formulário'}}</p>
        </div>
        <ul class="app-breadcrumb breadcrumb">
          <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
          <li class="breadcrumb-item"><a href="{{url('admin/aluno_faltas')}}">Faltas de Alunos</a></li>
        ...
        <div class="col-md-6">
          <div class="tile">
            <h3 class="tile-title">
              ...
              {{isset($item->id)?isset($show)?'Ver':'Editar':'Cadastrar'}} Faltas de Alunos</h3>
            
                {!! Form::open(['url' => 'admin/aluno_faltas']) !!}
                <div class="tile-body">   
                @if(isset($item->id))
                ...
                <div class="form-group">
                  ...
                  <select class="form-control {{ $errors->has('status')? 'is-invalid':'' }}" id="status" name="status">
                    <option value="">            
                      ...
                      <option value="1" {{ old('status') == '1' ? "selected='selected'" : isset($item->status) && $item->status == '1' ? "selected='selected'" : '' }}>
                        Sim
                      </option>
                      <option value="0" {{ old('status') == '0' ? "selected='selected'" : isset($item->status) && $item->status == '0' ? "selected='selected'" : '' }}>
                        Não
                      </option>                    
                  </select>
                  {!! $errors->has('status')? '<small id="passwordHelpBlock" class="form-text text-danger">'.$errors->first('status').'</small>':'' !!}
                  </div>  
            ...
            <div class="tile-footer">
              ...
              @else
              <a href="{{url('admin/aluno_faltas/'.$item->id.'/edit')}}" class="btn btn-secondary"><i class="fa fa-fw fa-lg fa-check-circle"></i>Editar</a>
              @endif
            </div>
              {{ Form::close() }}
          </div>
